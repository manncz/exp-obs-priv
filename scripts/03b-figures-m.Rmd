---
title: "Figures - Regression Estimator"
...
```{r}
load("sim-out/var-m/sim-result-10.Rdata")
load("sim-out/var-m/sim-result-20.Rdata")
load("sim-out/var-m/sim-result-50.Rdata")
```
...
table_1 <- plot.dat %>%
  ...
  mutate(RE_sd = case_when(RE_sd_se > .01 ~ sprintf("%1.2f (%1.2f)", RE_sd, RE_sd_se),
                           TRUE ~ sprintf("%1.2f", RE_sd)),
         RE_reg = case_when(RE_reg_se > .01 ~ sprintf("%1.2f (%1.2f)", RE_reg, RE_reg_se),
...
print(xtab, comment = F, size="footnotesize",
      file='../figures/var-sim-m-p10.tex', include.rownames = F)
```
...
xtab <- table_1 %>%
  ...
  xtable(caption = "", table.placement = "ht",
               digits = c(0,3,2,2))
print(xtab, comment = F, size="footnotesize",
      file='../figures/var-sim-m-p20.tex', include.rownames = F)
```
...
xtab <- table_1 %>%
  ...
  xtable(caption = "", table.placement = "ht",
               digits = c(0,3,2,2))
print(xtab, comment = F, size="footnotesize",
      file='../figures/var-sim-m-p50.tex', include.rownames = F)
```
